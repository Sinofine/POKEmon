...
# Local imports
from montgomery_isogenies.kummer_line_ext import KummerLine, KummerPoint
# =================================================== #
...
class KummerLineIsogeny_Velu(KummerLineIsogeny_Generic):
    ...
    def _compute_codomain_constants_even(self):
        ...
        A, C = self._domain.extract_constants()
        if not XK:
            return -2*A, (self._domain.base_ring()(A*A-4*C*C)).sqrt()
    ...
    def _evaluate_isogeny_even(self, P):
        ...
        if not XK:
            ...
            T9 = XP*ZP*(self._domain.base_ring()(A*A-4*C*C)).sqrt()
